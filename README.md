...
The SAP Cloud Platform, portal service for Partner Channel Management configuration guide provides all you need to deploy a PCM solution on your SCP account and connect it to your SAP Cloud for Customer tenant and Identity Authentication tenant.
...
*  pcmapps - an HTML5 application containing UI5 Components corresponding to SAP Cloud for Customer Business Objects such as Lead, Opportunity, Task etc. There are 3 types of applications: List, Object Details, and Create New Object.
...
*	In SAP Cloud for Customer, Partner Program Management project scope is enabled. For more information [read this blog](https://blogs.sap.com/2015/09/26/cloud-for-customer-specific-configurations-for-partner-channel-management/)
*	In SAP Cloud for Customer, Deal Registration is also selected if needed.
*	In SAP Cloud for Customer,	maintain the Business Roles in the Business Configuration
*	Users:
     -  Admin access to an SCP account
	 -  TENANT_ADMIN user for Portal Service
     -  SAP Cloud for Customer admin user
...
#### 2.6	Set up trust between the customer account and SAP Cloud for Customer backend (for SAP Cloud for Customer invitation flow)
...
Public SAP Cloud for Customer destination (used for connecting to the SAP Cloud for Customer backend during a guest registration scenario) with the parameters in the next section.
...
| Type | HTTP|
| Description | (Optional) Enter a meaningful description |
| URL | …/sap/byd/odata/v1/pcmportal (this relative path should come after the full URL to SAP Cloud for Customer tenant, E.g. https://myXXXXXX.crm.ondemand.com/sap/byd/odata/v1/pcmportal) |
| ProxyType | Internet |
| Authentication | BasicAuthentication |
| User | Admin user of the SAP Cloud for Customer tenant |
| Password | Admin password for the SAP Cloud for Customer tenant |  

#### 2.8	Configure SAP Cloud for Customer Backend
...
##### 2.8.2	Instructions
   1.	Enter the SAP Cloud for Customer PCM CRM, for example: https://my312033.crm.ondemand.com, as an administrator.
...
**II.** Authenticated URL. Data from SAP Cloud for Customer is available for Partner Managers:
https://flpnwc-[account name].dispatcher.[data center].hana.ondemand.com/sites/[site alias]?hc_login#Shell-home
  
**Note:** Please do not assign users directly to Partner Manager role in SCP . The assignment to this role is done automatically by the callback batch job in SAP Cloud for Customer, which runs every night in the background, after the user's application form is approved.
